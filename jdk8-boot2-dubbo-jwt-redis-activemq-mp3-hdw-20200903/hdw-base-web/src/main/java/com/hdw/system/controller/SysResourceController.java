...
public class SysResourceController {
    ...
    private void verifyForm(SysResource sysResource) {
        ...
        //菜单
        if (sysResource.getResourceType() == CommonEnum.MENU.getCode()) {
            if (StringUtils.isBlank(sysResource.getUrl())) {
                throw new GlobalException("菜单URL不能为空");
            }
        }
        //上级菜单类型
        int parentType = CommonEnum.CATALOG.getCode();
        ...
        //目录、菜单
        if (sysResource.getResourceType() == CommonEnum.CATALOG.getCode() ||
                sysResource.getResourceType() == CommonEnum.MENU.getCode()) {
            if (parentType != CommonEnum.CATALOG.getCode()) {
        ...
        //按钮
        if (sysResource.getResourceType() == CommonEnum.BUTTON.getCode()) {
            if (parentType != CommonEnum.MENU.getCode()) {
